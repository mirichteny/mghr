...
public class NonProjectCategory extends Category {
		...
		
	{
		...
		this.setNumDroppedGrades(newNumGradesToDrop);
	}
	...
	/* dropNGrades()
	 * @param gradeList an ArrayList of Doubles which are all the grades to consider
	 * @param numberToDrop an int representin the number of grades to drop from the result
	 ...
	 *  - modify this method however you need to run it succesfully
	...
	public ArrayList<Double> dropGrades( )
	{
		...
		{
			droppedList.add(new Double(grade));
		}
		
		int dropCounter = 0;
		while((this.getNumDroppedGrades() <= droppedList.size()) && (dropCounter < this.getNumDroppedGrades())) //Fix this code
		{
			int minIndex = 0;		
